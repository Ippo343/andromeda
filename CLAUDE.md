...
**Comms** — WiFi + web server. All runtime commands (NEXT, HOLD, POWER_OFF, POWER_ON, COLOR, BRIGHTNESS, MODEL, REBOOT, EFFECT) go over a single persistent WebSocket (`/ws`), parsed and queued to MissionControl. HTTP is only used for static files and one-shot provisioning (`/save`, `/reset`) and reads (`/fps`, `/brightness`). Effects have a stable `EffectId`/`EFFECT_REGISTRY` (mirrors `MODEL_REGISTRY`), letting the web UI select a specific effect by id (implicitly holding it) instead of only cycling the random rotation.
