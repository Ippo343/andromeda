...
{
    Command out;
    TEST_ASSERT_TRUE(WsCommandParser::parse("{\"type\":\"brightness\",\"value\":128}", out));
    TEST_ASSERT_EQUAL(static_cast<int>(CommandType::BRIGHTNESS), static_cast<int>(out.type));
    TEST_ASSERT_EQUAL(128, out.brightness);
}

void test_rejects_brightness_out_of_range()
{
    Command out;
    TEST_ASSERT_FALSE(WsCommandParser::parse("{\"type\":\"brightness\",\"value\":-1}", out));
    TEST_ASSERT_FALSE(WsCommandParser::parse("{\"type\":\"brightness\",\"value\":256}", out));
}
