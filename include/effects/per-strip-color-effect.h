...
// in precompute()" - SaturationGlow and IndividualStripDrift both reduce to this shape
// at render time, whether or not they need extra per-strip timing/interpolation state
// (transition start/end times, previous colors, ...) to arrive at that color.
// Centralizing render() also lets it use fill_solid() instead of a virtual evaluate()
// call per LED.
